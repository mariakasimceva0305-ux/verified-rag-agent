...
Система question answering по документам на основе RAG: индексация корпуса, retrieval релевантных фрагментов, ответ с цитатами и сравнение базового и улучшенного режимов пайплайна.

## Краткий обзор

- Основной фокус: retrieval, цитирование источников, простая проверка groundedness.
- В репозитории есть встроенный тестовый корпус и профили `baseline` / `improved`.
...
Проект решает задачу поиска ответов по внутренней базе документов с проверяемыми ссылками на источники.

- Четкий pipeline ingestion -> retrieval -> answer.
- Сравнение baseline и improved конфигураций.
- Воспроизводимый локальный запуск и офлайн-оценка.
